...
<div class="e2-note <?= $note['hidden?']? 'e2-note-hidden' : '' ?> <?= $note['favourite?']? 'e2-note-favourite' : '' ?>" data-note-id="<?= $note['id'] ?>" <?php if (!empty ($note['read-href'])) { ?> data-note-read-href="<?= $note['read-href'] ?>" <?php } ?>>
    ...
    <article>
        <?php if (@$note['userpic-href']) { ?>
            <div class="e2-note-author-picture"><img src="<?= $note['userpic-href'] ?>" alt="<?= @$note['source'] ?>" /></div>
        <?php } ?>

        <?php if (@$note['author']) { ?>
            <div class="e2-note-author-name"><?= @$note['author'] ?></div>
        <?php } ?>

        <?php if ($note['draft?']) { ?>
            <div class="e2-nonpublic-label"><?= _S ('gs--not-published') ?></div>
        <?php } ?>

        <?php if ($note['scheduled?']) { ?>
            <div class="e2-nonpublic-label"><?= _S ('gs--will-be-published') ?> <?=_DT ('j {month-g} Y, H:i', @$note['time'])?></div>
        <?php } ?>
    ...
    <div class="e2-note-meta">
        <?php if ($note['comments-link?']): ?>
            <?php if ($note['comments-count']) { ?>
                <a href="<?= $note['href-comments'] ?>" class="nu"><span class="e2-svgi"><?= _SVG ('comments') ?></span> <u><?= $note['comments-count-text'] ?></u></a><?php if ($note['new-comments-count'] == 1 and $note['comments-count'] == 1) { ?>,
                <?= _S ('gs--comments-all-one-new') ?><?php } elseif ($note['new-comments-count'] == $note['comments-count']) { ?>,
                <?= _S ('gs--comments-all-new') ?><?php } elseif ($note['new-comments-count']) { ?> · <span class="admin-links"><a href="<?=$note['href']?>#new"><?= $note['new-comments-count-text'] ?></a></span><?php } ?>
            <?php } else { ?>
